...
describe('fnOS package metadata', () => {
  ...
  it('keeps the native manifest version separate from the Web UI package version', () => {
    ...
    expect(readFnOsFile('cmd', 'main')).toContain(`HERMES_FNOS_NATIVE_VERSION="${manifest.version}"`)
  ...
  it('uses Chinese fnOS lifecycle wizards for install, upgrade, config, and uninstall', () => {
    ...
    expect(readFnOsFile('wizard', 'uninstall')).toContain('"label": "保留对话、用户配置和模型配置"')
    expect(readFnOsFile('wizard', 'uninstall')).toContain('模型配置')
